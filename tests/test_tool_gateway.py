...
def test_log_interaction_persists_record_in_store():
    payload = base_payload("req-interaction-1", "log_interaction")
    response = client.post("/tools/crm/log_interaction", json=payload)

    assert response.status_code == 200
    assert "req-interaction-1" in store.interactions
