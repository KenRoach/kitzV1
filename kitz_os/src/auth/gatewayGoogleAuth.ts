 ...
 * Copied from kitz-gateway/src/googleAuth.ts for embedding auth routes in kitz-os.
 *
 * Flow:
 *   1. Frontend calls GET /api/gateway/auth/google/url → returns Google consent URL
 *   2. User consents, Google redirects to frontend /login?code=...
 *   3. Frontend sends code to POST /api/gateway/auth/google/callback
 *   4. kitz-os exchanges code → gets userinfo → returns JWT
 */
