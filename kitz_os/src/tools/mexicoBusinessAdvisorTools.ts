...
export function getAllMexicoBusinessAdvisorTools(): ToolSchema[] {
  return [{ name: 'mexico_business_advise', description: 'Mexico business advice: RFC, SAT, RESICO regime, IMSS, INFONAVIT, ISR/IVA, CFDI electronic invoicing, labor law.', parameters: { type: 'object', properties: { business_type: { type: 'string', description: 'Type of business' }, regime: { type: 'string', description: 'Tax regime (RESICO, general, etc.)' }, employees: { type: 'number', description: 'Number of employees' }, question: { type: 'string', description: 'Specific question' } }, required: ['business_type'] }, riskLevel: 'low' as const, execute: async (args, traceId) => { const raw = await callLLM(`Mexico: ${args.business_type}\nRegime: ${args.regime || 'RESICO'}\nEmployees: ${args.employees || 0}${args.question ? `\nQ: ${args.question}` : ''}`); let parsed; try { const m = raw.match(/\{[\s\S]*\}/); parsed = m ? JSON.parse(m[0]) : { error: 'Parse failed' }; } catch { parsed = { raw: raw }; } console.log(JSON.stringify({ ts: new Date().toISOString(), module: 'mexicoBusinessAdvisorTools', trace_id: traceId })); return parsed; } }];
}
