...
export function getAllWorkflowDesignerTools(): ToolSchema[] {
  return [{ name: 'workflow_design', description: 'Design automation workflows using n8n, Zapier, Make', parameters: { type: "object", properties: { business: { type: "string", description: "Business name" }, process: { type: "string", description: "Process to automate" }, current_steps: { type: "string", description: "Current manual steps" } }, required: ["business", "process"] }, riskLevel: 'low' as const, execute: async (args, traceId) => { const raw = await callLLM(JSON.stringify(args)); let p; try { const m = raw.match(/\{[\s\S]*\}/); p = m ? JSON.parse(m[0]) : { error: 'Parse failed' }; } catch { p = { raw }; } console.log(JSON.stringify({ ts: new Date().toISOString(), module: 'workflowDesignerTools', trace_id: traceId })); return p; } }];
}
