...
export function getAllPodcastAdvisorTools(): ToolSchema[] {
  return [{ name: 'podcast_advise', description: 'Podcast launch, production, growth, monetization', parameters: { type: "object", properties: { topic: { type: "string", description: "Podcast topic" }, target_audience: { type: "string", description: "Target audience" }, budget: { type: "string", description: "Budget" } }, required: ["topic"] }, riskLevel: 'low' as const, execute: async (args, traceId) => { const raw = await callLLM(JSON.stringify(args)); let p; try { const m = raw.match(/\{[\s\S]*\}/); p = m ? JSON.parse(m[0]) : { error: 'Parse failed' }; } catch { p = { raw }; } console.log(JSON.stringify({ ts: new Date().toISOString(), module: 'podcastAdvisorTools', trace_id: traceId })); return p; } }];
}
