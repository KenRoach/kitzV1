...
const GAME_NAMES: Record<number, string> = { 1: 'Startup', 2: 'Growth', 3: 'Scale' }
  ...
  return (
    ...
    >
      ...
      <div className="mb-8 flex items-center gap-2">
        ...
        <span className="text-[7px] tracking-[0.3em]" style={{ color: '#FBBF2480' }}>REVENUE LEADERS</span>
        <div className="h-px w-12" style={{ background: 'linear-gradient(90deg, transparent, #FBBF2460, transparent)' }} />
      </div>
      ...
      >
        ...
        >
          ...
          <span className="w-16 text-center">Game</span>
          <span className="w-24 text-right">Revenue</span>
        </div>

        {sorted.length === 0 ? (
          <div className="flex flex-col items-center py-12">
            <p className="text-[8px]" style={{ color: '#4a4a6a' }}>NO FOUNDERS YET</p>
            <p className="mt-2 text-[6px]" style={{ color: '#333355' }}>START YOUR FIRST BUSINESS</p>
          </div>
        ) : (
          sorted.map((e, i) => {
            ...
            return (
              ...
              >
                ...
                <span className="flex-1 text-[8px] text-white">{e.name}</span>
                <span className="w-16 text-center text-[7px]" style={{ color: '#64748b' }}>
                  {GAME_NAMES[e.world] ?? `G${e.world}`}
                </span>
                <span
                  className="w-24 text-right text-[9px] font-bold"
                  style={{
      ...
      <button
        onClick={onBack}
        className="mt-8 px-8 py-3 text-[9px] font-bold uppercase tracking-[0.3em] transition-all hover:scale-105 active:scale-95"
        style={{
          color: '#C084FC',
